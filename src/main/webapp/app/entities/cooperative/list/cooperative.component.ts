...
import { EntityArrayResponseType, CooperativeService } from '../service/cooperative.service';
...
export class CooperativeComponent implements OnInit {
  ...
  trackId = (_index: number, item: ICooperative): number => this.cooperativeService.getCooperativeIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.cooperatives = this.refineData(dataFromBody);
  }

  protected refineData(data: ICooperative[]): ICooperative[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ICooperative[] | null): ICooperative[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.cooperativeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
