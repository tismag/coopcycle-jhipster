...
import { EntityArrayResponseType, CommercantService } from '../service/commercant.service';
...
export class CommercantComponent implements OnInit {
  ...
  trackId = (_index: number, item: ICommercant): number => this.commercantService.getCommercantIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.commercants = this.refineData(dataFromBody);
  }

  protected refineData(data: ICommercant[]): ICommercant[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ICommercant[] | null): ICommercant[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.commercantService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
