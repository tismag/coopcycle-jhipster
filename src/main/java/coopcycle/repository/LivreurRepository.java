...
public interface LivreurRepository extends ReactiveCrudRepository<Livreur, Long>, LivreurRepositoryInternal {
    @Query("SELECT * FROM livreur entity WHERE entity.cooperative_id = :id")
    Flux<Livreur> findByCooperative(Long id);

    @Query("SELECT * FROM livreur entity WHERE entity.cooperative_id IS NULL")
    Flux<Livreur> findAllWhereCooperativeIsNull();
